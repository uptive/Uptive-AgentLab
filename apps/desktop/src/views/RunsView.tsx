...
const DEFAULT_INPUT = JSON.stringify({ pullRequestId: 42, repository: "acme/checkout" }, null, 2);
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 20 }}>
          ...
          <button
            ...
            onClick={submit}
            disabled={!canStart}
            style={{ ...buttonStyle("primary"), opacity: canStart ? 1 : 0.5, cursor: canStart ? "pointer" : "not-allowed" }}
          >
            ▶ Run
          </button>
        </div>
...
export function RunsView() {
  ...
  const handleStart = useCallback(
    (flow: FlowDefinition, input: unknown) => {
      const runId = startRun(flow, input, resolveAgent);
      setNewRunOpen(false);
  ...
  const handleRerun = useCallback(
    (run: Run) => {
      ...
      handleStart(resolveRunFlow(run, catalog), input);
    },

...
export function RunGraph({ flow, run, agentsById, selectedNodeId, onSelectNode, now }: Props) {
  ...
  const ctx = useMemo<EditorContextValue>(() => {
    ...
    // Read-only view: no local-agent warnings, and built-in agents count as shared.
    const sourced = new Map(
      [...agentsById].map(([id, agent]) => [id, { ...agent, source: (agent as Partial<SourcedAgent>).source ?? "database" } as SourcedAgent]),
    );
    return { agentsById: sourced, cloud: false, incoming, invalidNodeIds: new Set(), demo: runFrame(flow, run, now) };
